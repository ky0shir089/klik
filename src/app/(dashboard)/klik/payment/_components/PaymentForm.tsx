...
const PaymentForm = ({ data }: iAppProps) => {
  ...
  const sumUnitAmount = useMemo(() => {
    ...
    return data.units
      ...
      .reduce((acc: number, unit: { amount: number }) => acc + unit.amount, 0);
  }, [selectedUnitIds, data.units]);
  ...
  const onSubmit = useCallback(
    ...
    [form, router, sumRvAmount, sumUnitAmount]
  );
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Table>
                ...
                <TableBody>
                  {data.rvs.map((rv: Pick<customerShowType, "rvs"[0]>) => (
                    <TableRow key={rv.id}>
                      <TableCell>
                        <FormField
            ...
            <Table>
              ...
              <TableBody>
                {data.units.map((item: Pick<customerShowType, "units"[0]>) => (
